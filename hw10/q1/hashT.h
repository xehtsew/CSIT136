...
template <class elemType> void hashT<elemType>::insert(int hashIndex, const elemType& rec) {
	int pCount;
	int inc;
...
void hashT<elemType>::search(int& hashIndex, const elemType& rec, bool& found) const {
	///////////////copied mostly from hashT::insert//////////////////////////
	int pCount = 0;
	int inc = 1;
	...
	//if the hash index is 1, and the item is at that index, found is true
...
template <class elemType> void hashT<elemType>::retrieve(int hashIndex, elemType& rec) const {
	if (indexStatusList[hashIndex] == 1) {
...
template <class elemType> void hashT<elemType>::print() const {
	//copied and edited from your "Closed hashing.h"
	for (int index = 0; index < HTSize - 1; index++) {
		if (indexStatusList[index] == 1)
			cout << HTable[index];
		else
			cout << "-";
			cout << ", ";
	} if (indexStatusList[HTSize - 1] == 1)
		cout << HTable[HTSize - 1];
	else
		cout << "-";
}

template <class elemType> hashT<elemType>::hashT(int size) {
	if (size > 0) {
