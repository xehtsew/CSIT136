...
{
public:
	elemType getCandidate(const elemType& searchItem) const;
...
template <class elemType>
elemType bSearchTreeType<elemType>::getCandidate(const elemType& searchItem) const {
    ...
    if (root == NULL)
        ...
        while (current != NULL) {
            ...
            if (current->info == insertItem) {
				...
				////IF ITS FOUND, ADD ANOTHER FUNCTION CALLED UPDATE
				///AND += VOTES
				///it will look just like insert.
				///then to find winner, look for largest element in tree.
