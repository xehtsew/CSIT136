...
public:
   ...
   void insertEnd(const elemType& insertItem);
	...
	int recursiveSearch(const elemType& item, int location) const;
...
int arrayListType<elemType>::recursiveSeqSearch(const elemType& item) const{
	return recursiveSearch(item, 0);
}


template <class elemType>
int arrayListType<elemType>::recursiveSearch(const elemType& item, int location) const {
	bool found = false;

	if (list[location] == item) {
		found = true;
	} else {
		return recursiveSearch(item, location + 1);
	}
	if (found) {
		return location;
	}
