...
template <class Type> void linkedListType<Type>::swap(nodeType<Type> *first, nodeType<Type> *second) {
	Type temp = first->info;
	first->info = second->info;
	second->info = temp;

}//end swap

template <class Type> void linkedListType<Type>::selectionSort() {

	nodeType<Type> *start = first;
	nodeType<Type> *move;
	nodeType<Type> *minimum;
	
	while (start->link != NULL) {
		minimum = start;
		move = start->link;
		while (move != NULL) {
			if (minimum->info > move->info) {
				minimum = move;
			}
			move = move->link;
		}
