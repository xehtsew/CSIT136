...
//*********************************************************************************************************
	Type howManyInList(const Type& countItem);
	//function to find how many of an object are in the list
...
template <class Type>
Type linkedListType<Type>::howManyInList(const Type& countItem) {
	int count = 0; //count to count the items.

	nodeType<Type> *current; //pointer to traverse the list

	current = first;    //set current so that it points to the first node
	while (current != NULL) //while theres more data to review
	{
		if (current->info == countItem) { //if currents info equals the item to search for
			count++							//add one to count.
		}
		current = current->link;		//update current's link
	}
