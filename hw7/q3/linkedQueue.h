...
const linkedQueueType<Type>& linkedQueueType<Type>::operator=(const linkedQueueType<Type>& otherQueue){
	if (this != &otherQueue) { //avoid self-copy
		copyQueue(otherQueue); //doesn't have to be null, as the whole thing will be replaced.
	}
		return *this;
...
linkedQueueType<Type>::linkedQueueType(const linkedQueueType<Type>& otherQueue) {
	queueFront = NULL;
	queueRear = NULL;
	copyQueue(otherQueue); //initialize front and rear, then call copyQueue.
}//end copy constructor

template <class Type>
void linkedQueueType<Type>::copyQueue(const linkedQueueType<Type>& otherQueue) {
	nodeType<Type> *newInfo;
	nodeType<Type> *currentLocation;

	//if there is something in the queue, delete the whole queue
	if (queueFront != NULL) { 
		deleteQueue();
	}
	//if the front is null, initialize the whole queue to null
	if (otherQueue.queueFront == NULL) {
		queueFront = NULL;
		queueRear = NULL;
	} else {
		//currentLocation becomes the front
		currentLocation = otherQueue.queueFront;

		queueFront = new nodeType<Type>;
		queueFront->info = currentLocation->info;
		queueFront->link = NULL;
		queueRear = queueFront;
		currentLocation = currentLocation->link;
		//while there are more locations in the queue, continue to copy until currentLocation links to NULL.
		while (currentLocation != NULL) {
			newInfo = new nodeType<Type>;
			newInfo->info = currentLocation->info;
			newInfo->link = NULL;
			queueRear->link = newInfo;
			queueRear = newInfo;
			currentLocation = currentLocation->link;
		}
	}
